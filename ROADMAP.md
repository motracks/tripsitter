...
| **Expand VISA_OPTIONS coverage** | Currently seeded with Nepal only. `api/research-visa-options.js` drafts candidate duration/fee tiers from official pages for Thailand, India, Indonesia, EU/Schengen (for a US citizen), and the US (for an EU citizen) — hit it manually, verify each result's `quote` against the source, then paste vetted entries into `VISA_OPTIONS` in `index.html`. A live autonomous scraper isn't feasible (see commit notes): pages are prose, redesign without notice, and often gate real fee tables behind a nationality form — a human check before shipping is load-bearing, not optional. | Small per country, manual review each time |
